...
static int inv_init_config(struct iio_dev *indio_dev)
{
    ...
    printk(" inv_init_config \n");
	if (st->chip_config.is_asleep)
		return -EPERM;

	reg = &st->reg;
    if(!reg){
        printk(" inv_init_config reg null \n");
        return -EPERM;
    }
...
static int inv_check_chip_type(struct inv_mpu_iio_s *st)
{
	...
	if (result){
        printk(" check chip setpower off erro \n");
		return result;
     };
	result = st->set_power_state(st, true);
	if (result){
        printk(" check chip setpower on erro \n");
		return result;
     }
...
static struct mpu_platform_data mpu6050_plat_data = {	
    .int_config  = 0x10,    
    .level_shifter = 0, 
    .orientation = {  0,  -1,  0,             
                    -1,  0,  0,             
...
#if defined(FEATURE_E1005)
    .secondary_orientation= {
				0,    1,    0,
				1 ,	0,    0,
				0,	0,   -1 },
#elif defined (FEATURE_E8005)
...
//--yd 	indio_dev = iio_allocate_device(sizeof(*st));
    ...
    client->irq = gpio_to_irq(155);
	st->client = client;
   	st->sl_handle = client->adapter;
	st->i2c_addr = MPU6050_ID;
    st->plat_data = mpu6050_plat_data;
    st->chip_type =INV_MPU6050;
	//	*(struct mpu_platform_data *)dev_get_platdata(&client->dev);
	
	/* power is turned on inside check chip type*/
	result = inv_check_chip_type(st);
	if (result)
		goto out_free;
    if(st->init_config == NULL) {
		result =  -ENOMEM;
		goto out_no_free;
	}
...
//	if (!strcmp(temp_id->name, "mpu6xxx")
...
//	else
	...
	if (result) {
		pr_err(" ring buffer register fail result=%x \n",result);
		goto out_unreg_ring;
	}
...
static struct acpi_device_id mpu6050_acpi_match[] = {
	{ "INVN6500", 0 },
	{ },
};
