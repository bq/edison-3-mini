...
#define VIBRATOR_ENABLE_PIN (95) //SIO_PWM[1]
...
static int vibr_Enable_HW(int on)
{
    int error = 0;

    gpio_set_value(VIBRATOR_ENABLE_PIN, on);
#if 0
    {
        int value = 0;
        msleep(20);
        value = gpio_get_value(VIBRATOR_ENABLE_PIN);
        printk("[vibrator]vibr_Enable3 value = %d\n", value);
    }
#endif
