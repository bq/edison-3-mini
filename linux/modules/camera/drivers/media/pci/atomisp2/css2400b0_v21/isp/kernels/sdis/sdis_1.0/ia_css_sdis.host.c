	...
	short *vertical_coefficients)
{
	...
	for (i = 0; i < (((dvs_binary)->info->sp.pipeline.isp_pipe_version == 2) ? IA_CSS_DVS2_NUM_COEF_TYPES : IA_CSS_DVS_NUM_COEF_TYPES); i++) {
...
ia_css_sdis_ver_coef_tbl_bytes(
	const struct ia_css_binary *binary)
{
	return sizeof(short) * (((binary)->info->sp.pipeline.isp_pipe_version == 2) ? IA_CSS_DVS2_NUM_COEF_TYPES : IA_CSS_DVS_NUM_COEF_TYPES) * binary->dis.coef.pad.height;
}
	...
	unsigned enabled)
{
	...
	dis->coef.pad.width  =
			_ISP_VECS(_ISP_BQS(sc_3a_dis_padded_width)) * ISP_VEC_NELEMS;
	dis->coef.pad.height =
			_ISP_VECS(_ISP_BQS(sc_3a_dis_height)) * ISP_VEC_NELEMS;
